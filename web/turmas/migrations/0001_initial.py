# Generated by Django 2.1.5 on 2019-03-14 14:27

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('escolas', '0001_initial'),
        ('ambientes', '0002_auto_20190314_1324'),
    ]

    operations = [
        migrations.CreateModel(
            name='Turmas',
            fields=[
                ('dre', models.TextField(blank=True, null=True)),
                ('codinep', models.IntegerField(blank=True, null=True)),
                ('tipoesc', models.CharField(blank=True, max_length=12, null=True)),
                ('nomesc', models.CharField(blank=True, max_length=60, null=True)),
                ('subpref', models.CharField(blank=True, max_length=35, null=True)),
                ('distrito', models.TextField(blank=True, null=True)),
                ('setor', models.SmallIntegerField(blank=True, null=True)),
                ('dturnos', models.TextField(blank=True, null=True)),
                ('t2d3d', models.TextField(blank=True, null=True)),
                ('numsala', models.CharField(blank=True, max_length=6, null=True)),
                ('descamb', models.CharField(blank=True, max_length=70, null=True)),
                ('capreal', models.IntegerField(blank=True, null=True)),
                ('metragem', models.IntegerField(blank=True, null=True)),
                ('codturm', models.IntegerField(primary_key=True, serialize=False)),
                ('ano', models.IntegerField(blank=True, null=True)),
                ('modal', models.CharField(blank=True, max_length=60, null=True)),
                ('semestre', models.IntegerField(blank=True, null=True)),
                ('rede', models.TextField(blank=True, null=True)),
                ('cdserie', models.IntegerField(blank=True, null=True)),
                ('descserie', models.CharField(blank=True, max_length=40, null=True)),
                ('turno', models.IntegerField(blank=True, null=True)),
                ('turma', models.CharField(blank=True, max_length=15, null=True)),
                ('vagofer', models.IntegerField(blank=True, null=True)),
                ('matric', models.BigIntegerField(blank=True, null=True)),
                ('alu_teg', models.BigIntegerField(blank=True, null=True)),
                ('alu_teg_def', models.BigIntegerField(blank=True, null=True)),
                ('hora_inicio', models.CharField(blank=True, max_length=4, null=True)),
                ('hora_final', models.CharField(blank=True, max_length=4, null=True)),
                ('dt_inclusao', models.DateTimeField(blank=True, null=True)),
                ('localizacao', models.CharField(blank=True, max_length=6, null=True)),
                ('classes', models.IntegerField(blank=True, null=True)),
                ('x_semana', models.BigIntegerField(blank=True, null=True)),
                ('durac_dia', models.IntegerField(blank=True, null=True)),
                ('durac_semana', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('escolarizacao', models.TextField(blank=True, null=True)),
                ('dias', models.TextField(blank=True, null=True)),
                ('al_reprov', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('al_evad', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('al_aprov', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('al_fale', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('al_excl', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('al_ncom', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('al_nrem', models.DecimalField(blank=True, decimal_places=65535, max_digits=65535, null=True)),
                ('pnee_ah', models.BigIntegerField(blank=True, null=True)),
                ('pnee_da', models.BigIntegerField(blank=True, null=True)),
                ('pnee_df', models.BigIntegerField(blank=True, null=True)),
                ('pnee_dv', models.BigIntegerField(blank=True, null=True)),
                ('pnee_di', models.BigIntegerField(blank=True, null=True)),
                ('pnee_tgd', models.BigIntegerField(blank=True, null=True)),
                ('pnee_bv', models.BigIntegerField(blank=True, null=True)),
                ('pnee_ba', models.BigIntegerField(blank=True, null=True)),
                ('pnee_tdi', models.BigIntegerField(blank=True, null=True)),
                ('pnee_mul', models.BigIntegerField(blank=True, null=True)),
                ('pnee_suce', models.BigIntegerField(blank=True, null=True)),
                ('cadeir', models.BigIntegerField(blank=True, null=True)),
                ('pnee_tot', models.BigIntegerField(blank=True, null=True)),
                ('database', models.DateField(blank=True, null=True)),
                ('codamb', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.DO_NOTHING, to='ambientes.AmbientesUnidadesEdu')),
                ('codesc', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.DO_NOTHING, to='escolas.Escolas')),
            ],
        ),
    ]
